...
static void calculate_setpoint_target(data *d) {
    ...
    if ((input_voltage < d->float_conf.tiltback_hv) &&
        !bms_is_fault_set(d->bms_fault, CELL_OVER_VOLTAGE)) {
    ...
    if (d->state.sat == SAT_CENTERING) {
        ...
        if (d->state.mode != MODE_FLYWHEEL) {
            d->state.sat = SAT_PB_DUTY;
               ...
               (input_voltage > d->float_conf.tiltback_hv ||
                bms_is_fault_set(d->bms_fault, CELL_OVER_VOLTAGE))) {
        if (bms_is_fault_set(d->bms_fault, CELL_OVER_VOLTAGE)) {
        ...
        // Setting type to HV to ensure that we trigger haptic buzz!
    ...
    } else if (bms_is_fault_set(d->bms_fault, CELL_OVER_TEMP) ||
               bms_is_fault_set(d->bms_fault, CELL_UNDER_TEMP) ||
               bms_is_fault_set(d->bms_fault, BMS_OVER_TEMP)) {
        // Use the angle from Low-Voltage tiltback, but slower speed from High-Voltage tiltback
        beep_alert(d, 3, true);
        if (bms_is_fault_set(d->bms_fault, CELL_OVER_TEMP)) {
            d->beep_reason = BEEP_TEMP_CELL_OVER;
        } else if (bms_is_fault_set(d->bms_fault, CELL_UNDER_TEMP)) {
        ...
        d->state.sat = SAT_PB_TEMPERATURE;
               ...
               (input_voltage < d->float_conf.tiltback_lv ||
                bms_is_fault_set(d->bms_fault, CELL_UNDER_VOLTAGE))) {
        beep_alert(d, 3, false);
        if (bms_is_fault_set(d->bms_fault, CELL_UNDER_VOLTAGE)) {
